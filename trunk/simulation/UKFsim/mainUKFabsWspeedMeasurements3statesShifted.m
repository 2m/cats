...
stream = RandStream.create('mt19937ar','seed',693);
...
nx=4;
stddegrees = 3;
lambda = [0, 0, stddegrees*(pi/180)] ;	% Standard deviation in radians
...
turnWait = 50; %value of m means m-1 steps wait
speed=0.013;
...
R = r(3)^2*eye(n+2);                          % covariance of measurement
R(1,1) = 1e+1;
R(2,2) = 1e+1;
Rfull=R;
f = @update;                                % nonlinear state equations
h = @measure2WspeedMeasurements2;            % measurement equation
...
for k=1:N
    ...
    for j=actLandm
        raysX{j,k}(1,:)=[x(1), cos(z(j,1))+x(1) - faraway*cos(z(j+2,1))];
        raysY{j,k}(1,:)=[x(2), sin(z(j,1))+x(2) - faraway*sin(z(j+2,1))];
    end
