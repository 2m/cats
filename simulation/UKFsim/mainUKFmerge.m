...
fov=43*pi/180;
stddegrees = 1;
lambda = [stddegrees*(pi/180),6e-3,6e-3,1e-6,1e-6];%[stddegrees*(pi/180),.5e-2,.5e-2,0,0] ;	% Standard deviation in radians
...
%         disp(i);
    ...
    for j=1:k-startplotidx
        ...
        plot(landm(:,1),landm(:,2),'*',...
            xVm(1,startplotidx+j:k),xVm(2,startplotidx+j:k),'-',...
            xVm(1,k),xVm(2,k),'.',...
            sVm(1,startplotidx+j:k),sVm(2,startplotidx+j:k),'-',...
            sVm(1,k),sVm(2,k),'.');
    end
    for i=1:nm
        for j=actLandm{1,i}
